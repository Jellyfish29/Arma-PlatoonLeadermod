...
if (pl_enable_map_icons_performance and pl_enable_map_icons) then {

    pl_draw_group_info_performance_selected = {
        findDisplay 12 displayCtrl 51 ctrlAddEventHandler ["Draw","
            _display = _this#0;
            {
                if (hcShownBar and (_x getVariable 'pl_show_info') and visibleMap) then {
    ...
    [] call pl_draw_group_info_performance_selected;

    pl_draw_group_info_performance_allways = {
        findDisplay 12 displayCtrl 51 ctrlAddEventHandler ["Draw","
            _display = _this#0;
            {
                if (hcShownBar and (_x getVariable 'pl_show_info') and visibleMap) then {
    ...
    [] call pl_draw_group_info_performance_allways;

        pl_mark_vics = {
        findDisplay 12 displayCtrl 51 ctrlAddEventHandler ["Draw","
            _display = _this#0;
            if (pl_show_vehicles and visibleMap) then {
                {
                    if ((_x distance2D pl_show_vehicles_pos < 150) or (_x isKindOf 'Air')) then {
                        if (((side _x) isEqualTo playerSide) or ((side _x) isEqualTo civilian)) then {
                            if ((_x isKindOf 'Tank') or (_x isKindOf 'Car') or (_x isKindOf 'Air') or (_x isKindOf 'Truck')) then {
