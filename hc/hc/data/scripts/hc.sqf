...
//--- Synchronization
waituntil {{typeof _x != "Pl_HighCommandSubordinate"} count (synchronizedObjects _logic) > 0 || _logic == bis_hc_mainscope};
...
for "_i" from 0 to (count _possibleLeaders - 1) do {
	...
	if (typeof _element == "Pl_HighCommandSubordinate") then {
...
if (count _sublogics > 0 && !_addAllGroups) then {
	...
	if (count _linkedLogic > 1) then {
		{
			if (typeof _x == "Pl_HighCommandSubordinate") then {
				_subordinate = _x;
...
if (_isMain) then {
	...
	[["hc\HC\data\scripts\HC_local.sqf"],"BIS_fnc_execVM",nil,true] call BIS_fnc_MP;
};
...
while {true} do {
	_allgroups = allgroups;
	{
		...
		if (isnil {_group getvariable "BIS_HC_fsmcoc"}) then {
			_fsm = _group execfsm ("hc\hc\data\scripts\HC_COC.fsm");
